...
Feature: Reports, Setup, Excel Templetes operations
  ...
  Scenario: Excel Templetes operations, create
    ...
    When  user create a excel templete name as "templete123" and period as 1
    Then  excel templete should be created

  Scenario: Excel Templetes operations, update
    Given user on excel templetes page
    When  user update the excel templete named "templete123" to name as "templete1123" and period as 1
    Then  excel templete should be updated

  Scenario: Excel Templetes operations, delete
    Given user on excel templetes page
    When  user delete the excel templete named as "templete1123"
    Then  excel templete should be deleted


  Scenario: Excel Templetes add, update and delete operations with rows and columns
    Given user on excel templetes page
    When  user create a excel templete name as "templete223" and period as 1 and row and column size as follows
      | 2 | 2 |
    Then  excel templete should be created

    When  user update the excel templete named "templete223" to name as "templete2223" and period as 1
    Then  excel templete should be updated

    When  user delete the excel templete named as "templete2223"
    Then  excel templete should be deleted
